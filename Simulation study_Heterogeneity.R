...
for (a in 1:1){
  ...
  onerun <- function(r){
    ...
    Y.1.1 <- X_1 + X_2 + X_3 - X_4 + X_5 + X_6 + 10 * (X_6 - 0.5) + rnorm(n_cohort, 0, 1)
    Y.2.0 <- X_1 + X_2 + 0.2 * X_3 * X_4 - sqrt(X_5) + rnorm(n_cohort, 0, 1)
    Y.2.1 <- X_1 + X_2 + 0.2 * X_3 * X_4 - sqrt(X_5) + 10 * (X_6 - 0.5) + rnorm(n_cohort, 0, 1)
    Y.3.0 <- (X_1 + X_2 + X_5)^2 + rnorm(n_cohort, 0, 1)
    Y.3.1 <- (X_1 + X_2 + X_5)^2 + 10 * (X_6 - 0.5) + rnorm(n_cohort, 0, 1)
  ...
  write.csv(sim.out, 
            paste0("Results_Het_Sigma", sigma^2, "_BalDsgn_", c, ".csv"))
}
